...
def handle_response(message) -> str:
    ...
    if cmd_dict.get(args[0]) != None:
        return cmd_dict[args[0]]
...
def load_commands():
    ...
    cursor = conn.execute("SELECT COMMAND, RET from COMMANDS;")
    for cmd in cursor:
        command = cmd[0]
        return_string = cmd[1]
        dict[command] = return_string
    return dict
...
def raid(args):
    try:
        ...
        if len(args) == 1: # !raid didnt specify any details
            return 'Please specify a pokemon name, or name + code + tera type. Examples:\n*!raid clodsire*\n*!raid clodsire EBU26B dark*'
